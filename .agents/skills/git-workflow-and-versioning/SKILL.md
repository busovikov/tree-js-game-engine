...
## @haku monorepo — commit format

**Only commit when the user explicitly asks** (project rule).
...
Summary for changes — complete sentences, why not just what.
```

Do **not** add `Co-authored-by: Cursor <cursoragent@cursor.com>`.

**Pre-commit verify** (minimal):

```bash
pnpm test          # or filtered package if small change
