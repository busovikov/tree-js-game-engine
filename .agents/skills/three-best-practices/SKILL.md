...
## @haku monorepo — use with project docs

**Pinned version:** Three.js `^0.171.0` (`packages/engine/package.json`) — not 0.182 from skill metadata.

**Read before engine work:**

| Doc | Purpose |
| --- | ------- |
| `docs/links.md` | Official Three.js URLs for 0.171 |
| `docs/architecture.md` | Simulation ≠ Presentation — no Three in components |
| `docs/edge-cases.md` | glTF race, render flags, editor-only passes |
| `RENDER_PLAN.md` | RenderGraph, feature flags, incremental phases |
| `docs/techstack.md` | Engine package layout |

**Code locations:** `packages/engine/src/render-sync/`, `render-backend.ts`, `mesh-factory.ts`, `model-loader.ts`

**Boundaries:** Three.js only in `@haku/engine` and editor viewport gizmos — never `@haku/core` or `@haku/schema`.
