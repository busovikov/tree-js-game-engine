...
## @haku monorepo — integration points

@haku does **not** set up raw `THREE.Scene` in apps — use `Engine` + `ThreeRenderBackend`:

| App | Entry |
| --- | ----- |
| Playground | `apps/playground/src/main.ts` — `Engine`, `SceneLoader` from `@haku/engine/runtime` |
| Editor viewport | `packages/editor/src/panels/ViewportPanel.tsx` — same engine path |

**Docs:** `docs/architecture.md` (render pipeline), `docs/links.md` (WebGLRenderer, OrbitControls URLs).

**Do not** put Three.js objects in React state. Editor orbit/gizmos: `packages/editor/src/viewport/`.
