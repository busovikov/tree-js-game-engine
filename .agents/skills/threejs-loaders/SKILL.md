...
- **Implementation:** `packages/engine/src/model-loader.ts` — `GLTFLoader`, `loadModelTemplate`, resolvers
- **Edge cases:** `docs/edge-cases.md` — async race (`modelLoadId`), stale callback
- **Editor wiring:** `projectService` → `setModelAssetResolver` in `ViewportPanel`
- **Official URL:** `docs/links.md` § GLTFLoader
- **Version:** Three.js ^0.171 (`packages/engine/package.json`)
