...
| Unlock, pause, or resume is requested repeatedly                   | `AudioRuntime` performs at most one successful backend transition for the current state; rejected transitions remain retryable through typed `AudioLifecycleError` |
| Bounce Run Start unlock rejects                                    | Keep the Start UI/session state and music stopped so the same real activation can retry                                                                            |
| Bounce Run bus volume is non-finite or outside 0–1                 | Reject before the graph/backend mutation; preserve the authored volume/mute setting                                                                                |
| Bounce Run pause/restart overlaps music playback                   | Suspend/resume the existing loop and keep at most one music voice; restart never duplicates it                                                                     |
...
| Audio pool participant is registered late and composition disposes | Unregister it after stopping voices; later pool leases retain no audio subscription                                                                                |
