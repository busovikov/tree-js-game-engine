...
function validateDraft(name: string, fields: readonly VisualComponentFieldDraft[]): string | null {
...
export const CustomComponentTypeDialog = memo(function CustomComponentTypeDialog({
  open,
  onCreate,
  onClose,
}: {
  ...
  const [fields, setFields] = useState<VisualComponentFieldDraft[]>([emptyNumberField()])
  const [error, setError] = useState<string | null>(null)
  const [saving, setSaving] = useState(false)
  const [includeSpeedControls, setIncludeSpeedControls] = useState(false)
  ...
  useEffect(() => {
    ...
    setError(null)
    setSaving(false)
    setIncludeSpeedControls(false)
  }, [open])

  const validation = useMemo(() => validateDraft(name, fields), [fields, name])
  const hasNumericSpeedField = fields.some(
    (field) => field.name.trim() === 'speed' && field.type === 'number',
  )

  if (!open) return null

  const updateField = (index: number, next: VisualComponentFieldDraft): void => {
    setFields((current) =>
      current.map((field, fieldIndex) => (fieldIndex === index ? next : field)),
    )
    setError(null)
  }
    ...
    >
      ...
      >
          ...
          >
            ...
            {field.type === 'number' ? (
              <NumberField
                ...
                value={field.default}
                disabled={saving}
                onChange={(value) => updateField(index, { ...field, default: value })}
              />
            ) : field.type === 'string' ? (
              <label className="mesh-field">
        ...
        <button
          type="button"
          disabled={saving}
          onClick={() => setFields((current) => [...current, emptyNumberField()])}
        >
          Add Field
        </button>
        <label className="mesh-field" style={{ marginTop: 12 }}>
          ...
          <input
            ...
            onChange={(event) => setIncludeSpeedControls(event.target.checked)}
          />
        </label>
        {!hasNumericSpeedField && (
          <p style={{ margin: '4px 0 0', color: '#aaa' }}>
            Add a numeric field named speed to enable the bundled extension example.
          </p>
        )}
